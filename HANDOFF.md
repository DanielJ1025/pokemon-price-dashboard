# 포켓몬 카드 대시보드 — 작업 핸드오프 (260924 갱신)

## ★ 260924 변경 (최신 — 아래 260909 내용보다 우선)
- **보유기록 원본 = 구글시트 `POKEMON-SHEETS` `_컬렉션` A1(JSON, `ts`=최종 저장시각 ms).** 페이지 열면 GET으로 불러오고, 변경 시 2.5초 뒤 POST. 충돌=`ts` 최신 우선(서버가 더 최신이면 409+최신본 → 클라이언트 채택). 가치 스냅샷(hist)은 ts 안 올림.
