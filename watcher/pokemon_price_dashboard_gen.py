...
_CACHE_DIR = Path(__file__).resolve().parent / ".imgcache"


def _raw(url):
    """원본 바이트 1회만 다운로드 → 디스크 캐시(.imgcache). 재생성 시 재다운로드 안 함."""
    if url not in _rawcache:
        cf = _CACHE_DIR / (hashlib.md5(url.encode()).hexdigest() + ".bin")
        if cf.is_file():
            _rawcache[url] = cf.read_bytes()
        else:
            try:
...
def thumb(url, box=(150, 210)):
    ...
    if raw:
        try:
            ...
            im.thumbnail(box)
            buf = io.BytesIO()
            im.save(buf, format="WEBP", quality=50)
...
def _tr(g, show_rarity=False, pack=""):
    link = f"https://collectory.cc/cards/{g['id']}" if g.get("id") else "#"
    im = thumb(g.get("img"), (120, 168))      # 행엔 40px로 축소표시, 클릭 라이트박스용
...
def section(disp, query, owned):
    ...
    meta = f"전체 {len(rows)}종 · 정규 [{base}] · 최고 🇰🇷 ₩{rows[0]['kr']:,}"
    return (f'  <section class="pack" data-base="{base}"><div class="pack-head"><h2>{H.escape(disp)}</h2>{own}'
            f'<span class="pack-meta">{meta}<span class="own-cnt"></span></span>'
            f'<div class="pack-prog"></div></div>'
            f'{table_html(rows, disp)}</section>')
...
.rr-col{flex:0 0 104px;display:inline-flex;align-items:center}
.rgrp>summary .chip{flex:0 0 auto}
...
.wsym{font-weight:400;color:var(--faint);opacity:.8;margin-right:1px}
th.wcolh{text-align:right}
...
.search:focus{border-color:var(--sar)}
...
.chip{font:inherit;font-size:11.5px;font-weight:700;color:var(--muted);background:var(--surface-2);border:1px solid var(--border);border-radius:999px;padding:4px 10px;cursor:pointer;transition:background .12s,color .12s}
.chip:hover{color:var(--text)}
.chip.on{background:var(--sar);color:#fff;border-color:var(--sar)}
...
.bkrow button:hover{border-color:var(--own)}
.bkmsg{font-size:12px;color:var(--own);font-weight:700}
...
.osort button:hover{color:var(--text)}
.osort button.on{background:var(--sar);color:#fff;border-color:var(--sar)}
...
.lb img{width:min(90vw,340px);height:auto;max-height:92vh;border-radius:12px;box-shadow:0 12px 48px rgba(0,0,0,.6)}
...
th.sorted{color:var(--sar)}th.sorted[data-dir="down"]::after{content:" ▾"}th.sorted[data-dir="up"]::after{content:" ▴"}
.osort .odir{background:var(--surface);border-color:var(--own);color:var(--own)}
...
_TABS_JS = '''(function(){
 ...
 let searchQ='',curTab=2,ownedSort='kr',ownedView='album',ownedDir=-1,ownedGroup='pack';const btns=[];
